    ...
    <section className="py-24 bg-white" id="rings">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center border-b border-gray-200 mb-12">
          <button 
            className={`px-8 py-4 text-[11px] font-bold tracking-[0.1em] uppercase transition-all relative ${
              activeTab === 'bestsellers' ? 'text-black' : 'text-gray-400 hover:text-gray-800'
            }`}
          ...
          <button 
            className={`px-8 py-4 text-[11px] font-bold tracking-[0.1em] uppercase transition-all relative ${
              activeTab === 'new' ? 'text-black' : 'text-gray-400 hover:text-gray-800'
            }`}
          ...
          {currentItems.map((item, index) => (
            <div key={item.id} className="group cursor-pointer relative">
              ...
              {activeTab === 'bestsellers' && index < 3 && (
                <div className="absolute -top-6 left-0 z-10 hidden sm:block">
                  <Crown 
                    ...
                    color={index === 0 ? '#DAA520' : index === 1 ? '#A9A9A9' : '#8B4513'} 
                    strokeWidth={1.5}
                  />
                </div>
              )}
              ...
              <div className="text-center">
                <h3 className="text-[11px] text-gray-500 mb-1 tracking-wider uppercase truncate px-2">{item.name}</h3>
                <p className="text-[11px] font-bold text-gray-900 mb-2 uppercase tracking-[0.15em]">{item.material}</p>
                <p className="text-[11px] text-gray-500 tracking-wider">RM {item.price.toFixed(2)}</p>
              </div>
            </div>
          ))}
        </div>
